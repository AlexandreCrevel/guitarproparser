...
impl Song {
    ...
    /// - ...
    /// - measure n/track m
    pub(crate) fn read_measures(&mut self, data: &[u8], seek: &mut usize) -> GpResult<()> {
    ...
    fn read_measure(&mut self, data: &[u8], seek: &mut usize, measure: &mut Measure, track_index: usize) -> GpResult<()> {
    ...
    fn read_measure_v5(&mut self, data: &[u8], seek: &mut usize, measure: &mut Measure, track_index: usize) -> GpResult<()> {
    ...
    fn read_voice(&mut self, data: &[u8], seek: &mut usize, voice: &mut Voice, start: &mut i64, track_index: usize) -> GpResult<()> {
    ...
    fn write_measure(&self, data: &mut Vec<u8>, track: usize, measure: usize, version: &(u8,u8,u8)) {
        if version.0 < 5 {self.write_voice(data, track, measure,0, version);}
            ...
            for v in 0..self.tracks[track].measures[measure].voices.len() {self.write_voice(data, track, measure,v, version);}
