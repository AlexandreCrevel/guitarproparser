...
fn test_audit_all_files() {
    ...
    let test_dir = if test_dir.exists() { test_dir } else { Path::new("./test") };
    ...
    let mut files: Vec<_> = fs::read_dir(test_dir).expect("Cannot read dir")
        .map(|e| e.unwrap().path())
        .filter(|p| p.is_file())
        .collect();
    files.sort();
...
fn test_let_it_be_gp3() {
    ...
    let path = if path.exists() { path } else { Path::new("./test/the-beatles-let_it_be.gp3") };
