...
pub(crate) fn read_byte_size_string(
    ...
    let length = read_byte(data, seek)?.to_usize().unwrap_or(0); // u8 to usize is always safe
...
pub(crate) fn read_color(data: &[u8], seek: &mut usize) -> GpResult<i32> {
    let r = read_byte(data, seek)?.to_i32().unwrap_or(0); // u8 to i32 is safe
    let g = read_byte(data, seek)?.to_i32().unwrap_or(0);
    let b = read_byte(data, seek)?.to_i32().unwrap_or(0);
    *seek += 1;
    Ok(r * 65536 + g * 256 + b)
}
