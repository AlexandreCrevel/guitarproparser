// GP3/4/5 implementation of effect operations
...
use crate::types::effects::tremolo::from_tremolo_value;
use crate::types::effects::trill::from_trill_period;
use crate::types::effects::velocity::*;
    ...
    fn read_harmonic(
        ...
        match read_signed_byte(data, seek)? {
            ...
            17 => {
                he.pitch = Some(PitchClass::from(
                    note.real_value(
                        &self.tracks[self.current_track.expect("Current track not defined")]
                            .strings,
                    )?,
                    None,
                    None,
        ...
        if let Some(h) = &note.effect.harmonic {
            ...
            if h.kind != HarmonicType::Artificial {
                if h.pitch.is_some() && h.octave.is_some() {
                    ...
                    if p.value == ((note.real_value(strings)? + 7) % 12) && o == Octave::Ottava {
                        byte = 15;
                    } else if p.value == (note.real_value(strings)? % 12)
                        && o == Octave::Quindicesima
                    {
                        byte = 17;
                    } else if p.value == (note.real_value(strings)? % 12) && o == Octave::Ottava {
                        byte = 22;
                    }
                } else {
                    byte = 22;
    ...
    fn write_harmonic_v5(
        ...
        if let Some(h) = &note.effect.harmonic {
            ...
            if h.kind == HarmonicType::Artificial && (h.pitch.is_none() || h.octave.is_none()) {
                ...
                write_byte(data, h.fret.unwrap().to_u8().unwrap());
            }
        }
        Ok(())
    }
