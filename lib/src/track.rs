...
impl Song {
    ...
    pub(crate) fn read_tracks(&mut self, data: &[u8], seek: &mut usize, track_count: usize) -> GpResult<()> {
        //println!("read_tracks()");
        for i in 0..track_count {self.read_track(data, seek, i)?;}
        Ok(())
    }

    pub(crate) fn read_tracks_v5(&mut self, data: &[u8], seek: &mut usize, track_count: usize) -> GpResult<()> {
