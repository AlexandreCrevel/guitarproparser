...
pub(crate) type GpResult<T> = Result<T, ParseError>;
...
pub(crate) fn read_byte(data: &[u8], seek: &mut usize ) -> GpResult<u8> {
    if data.len() <= *seek { return Err(ParseError::UnexpectedEof { position: *seek, needed: 1 }); }
    let b = data[*seek];
    *seek += 1;
    Ok(b)
}
...
pub(crate) fn read_short(data: &[u8], seek: &mut usize ) -> GpResult<i16> {
    if data.len() < *seek + 2 { return Err(ParseError::UnexpectedEof { position: *seek, needed: 2 }); }
...
pub(crate) fn read_int(data: &[u8], seek: &mut usize ) -> GpResult<i32> {
    if data.len() < *seek + 4 { return Err(ParseError::UnexpectedEof { position: *seek, needed: 4 }); }
...
pub(crate) fn read_double(data: &[u8], seek: &mut usize ) -> GpResult<f64> {
    if data.len() < *seek + 8 { return Err(ParseError::UnexpectedEof { position: *seek, needed: 8 }); }
...
pub(crate) fn read_int_size_string(data: &[u8], seek: &mut usize) -> GpResult<String> {
    let size = read_int(data, seek)?.to_usize().ok_or(ParseError::IoError("invalid string size".into()))?;
...
pub(crate) fn read_int_byte_size_string(data: &[u8], seek: &mut usize) -> GpResult<String> {
    let s = (read_int(data, seek)? - 1).to_usize().ok_or(ParseError::IoError("invalid string size".into()))?;
...
pub(crate) fn read_byte_size_string(data: &[u8], seek: &mut usize, size: usize) -> GpResult<String> {
    let length = read_byte(data, seek)?.to_usize().ok_or(ParseError::IoError("invalid string length".into()))?;
...
fn read_string(data: &[u8], seek: &mut usize, size: usize, length: Option<usize>) -> GpResult<String> {
    let length = length.unwrap_or(size);
    if data.len() < *seek + length {
        return Err(ParseError::UnexpectedEof { position: *seek, needed: length });
    }
    ...
    if had_errors {
        let parse = std::str::from_utf8(&data[*seek..*seek+length])
            .map_err(|_| ParseError::EncodingError { position: *seek })?;
        *seek += size;
        return Ok(parse.to_string());
    }
    *seek += size;
    Ok(cow.to_string())
}
...
/// * returns version
pub(crate) fn read_version_string(data: &[u8], seek: &mut usize) -> GpResult<crate::headers::Version> {
...
mod test {
    ...
    fn test_read_byte_size_string() {
        let data: Vec<u8> = vec![0x18,0x46,0x49,0x43,0x48,0x49,0x45,0x52,
                                 ...
                                 0x30];
        let mut seek = 0usize;
        assert_eq!(read_byte_size_string(&data, &mut seek, 30).unwrap(), "FICHIER GUITAR PRO v3.00");
    }
    ...
    fn test_read_int_byte_size_string() {
        ...
        let mut seek = 0usize;
        assert_eq!(read_int_byte_size_string(&data, &mut seek).unwrap(), "%ARTIST%");
    }
